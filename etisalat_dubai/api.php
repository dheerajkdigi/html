...
$logFilePath = '/var/log/billing/' . date('Y') . '/' . date('m') . '/etisalat_dubai/api_' . date('Ymd') . '.log';
...
if(!($ext_trans_id && $msisdn && $plan) || !in_array($action,array('sub','status')) || ($action == "sub" && !($aff_id && in_array($other1,array("send_otp", "verify_otp")) && !($other1 == "verify_otp" && $other2 == "") )) ) {
	$aData = array('msisdn' => $msisdn, 'amount' => '', 'trans_id' => $ext_trans_id, 'result' => array('status' => "FAIL", 'code' => "SS400", 'message' => "Bad Request"), 'operator' => array('name' => 'etisalat_dubai', 'circle' => $paramArray['circle']));
